...
class TeachingModule extends React.Component {

  state = {
    date: new Date(),
    visibleIndex: 0, // Tracking which of the elements is currently shown
  ...
  canMoveNext = () => {
    return !!(this.state.selectedAnswer && this.state.selectedAnswer.is_correct)
  }

  evaluateMoveNext = () => {
    const result = !!(this.state.selectedAnswer && this.state.selectedAnswer.is_correct)
    this.setState({ canMoveNext: result })
    return result
  }

  moveToNextElement = () => {
    if (this.canMoveNext()) {
      const nextElem = this.props.blok.body[this.state.visibleIndex + 1]
      if (nextElem.component === 'question') {
        this.setState({ selectedAnswer: {} }) // Making the answer empty so we can only move next afer correct answer has bene reported
      }
  ...
  isLast = () => {
    return this.props.blok.body.length - 1 === this.state.visibleIndex
  }

  noMeta = () => {
    alert(`Sorry guys. Doesn't do anything yet. Could not resist to least display the button as a preview of what could be next ;-)`)
  }
  
  render() {
    return (
      <div {...storyblokEditable(this.props.blok)}>
        <h2>{this.props.blok.headline}</h2>
        <div className="flex justify-end">
          <button 
            type="button" 
            className="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mb-2"
            style={{ display: this.isLast() ? 'inline-flex' : 'none' }}
            onClick={this.noMeta}
          >
        ...
        <StoryblokComponent 
          key={this.props.blok.body[this.state.visibleIndex]._uid} 
          blok={this.props.blok.body[this.state.visibleIndex]}
          onQuestionAnswered={this.handleQuestionAnswered}
        />
        <div className="flex justify-end">
          <button 
            type="button"
            className="text-white bg-gray-800 hover:bg-gray-900 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mt-4 focus:outline-none disabled:opacity-60" 
            style={{ display: !this.isLast() ? 'inline-flex' : 'none' }}
            onClick={this.moveToNextElement}
