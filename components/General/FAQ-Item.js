...
const FAQItem = ({ blok, parent, first, last }) => {
   ...
   <div>
     <div className={`flex flex-row ${borderClasses}`} style={styles}>
       <div className={`flex flex-col relative w-full ${parent.item_classes || 'py-7 space-y-3' }`}>
         <div className={`question-title ${parent.q_text_classes || 'text-xl font-bold'} pr-12`}>
           { blok.question }
         </div>
         <input type="checkbox" className={`peer absolute right-[2px] top-3 ${parent.button_sizes || 'h-[36px] w-[36px]'} opacity-0 cursor-pointer z-10`}/>
         <div className={`${parent.button_classes || 'bg-[#ececec] rounded-full' } ${parent.button_sizes || 'p-3'} transition-transform duration-500 rotate-0 peer-checked:rotate-180 absolute right-[1.5px] top-3`}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 256 256">
              <path d="M213.66,101.66l-80,80a8,8,0,0,1-11.32,0l-80-80A8,8,0,0,1,53.66,90.34L128,164.69l74.34-74.35a8,8,0,0,1,11.32,11.32Z" style={{ fill: 'currentColor', width: '100%', hegith: '100%' }}></path>
            </svg>
            <img
